...
class Problem_mod(ipopt.problem):
    """
    ...
    def hessianstructure(self):
        ...
        for q in range(self.n_orbitals):
            ...
            #Accuracy = 4
            # p'=p+2  
            for p in range(0,self.n_points-2):
                hess[q, p, q, p+2] = 1.
            #Accuracy = 6
            # p'=p+3 
            for p in range(0,self.n_points-3):
                hess[q, p, q, p+3] = 1.
            """
    ...
    def hessian(self, x, lagrange, obj_factor):
        ...
        for q in range(self.n_orbitals):
            ...
            """
            # Accuracy = 4
            diag += obj_factor * deg*(-T) * (2.*self.h*self.C0[q,:] - 5.*self.C2/self.h )
            ...
            # Off-diagonal terms
            plus = 4./3. * ( self.C1 + 2./self.h * self.C2 )  # p' = p+1
            rng = np.arange(self.n_points-1)
            hess[q, rng, q, rng+1] = plus[:-1]* obj_factor * deg*(-T)
            
            # Off-diagonal terms
            plus = -1./6. * ( self.C1 + self.C2 / self.h)  # p' = p+2
            rng = np.arange(self.n_points-2)
            hess[q, rng, q, rng+2] = plus[:-2]* obj_factor * deg*(-T)
            """
            
            # Accuracy = 6
            diag += obj_factor * deg*(-T) * (2*self.h*self.C0[q,:] - 49./9. * self.C2/self.h )
            #diag += obj_factor * deg*(-T) * 2. * self.C0[q,:]
            # Fill with the q=q, p=p' terms
            np.fill_diagonal( hess[q,:,q,:], diag )
            ...
            # Off-diagonal terms
            plus = 3 * ( self.C1/2. + self.C2/self.h)  # p' = p+1
            rng = np.arange(self.n_points-1)
            hess[q, rng, q, rng+1] = plus[:-1]* obj_factor * deg*(-T)
            
            # Off-diagonal terms
            plus = -3./10. * ( self.C1 + self.C2 / self.h)  # p' = p+2
            rng = np.arange(self.n_points-2)
            hess[q, rng, q, rng+2] = plus[:-2]* obj_factor * deg*(-T)
            
            # Off-diagonal terms
            plus = 1./15. * ( self.C1/2. + self.C2 / (3*self.h))  # p' = p+3
            rng = np.arange(self.n_points-3)
            hess[q, rng, q, rng+3] = plus[:-3]* obj_factor * deg*(-T)
