...
def E_behaviour_calc(prec="-8", n=0, max_iter=2000):
    ...
    #defining problem
    nucl = Problem(Z,N, max_iter=max_iter, rel_tol=1e-4, constr_viol=1e-4, \
                      data=data, basis=ShellModelBasis(), exact_hess=True)
        
    ###############################################################################
    # calculating energy for paramters between 0.3 and 0.99
    ...
    # calculating energy for paramters between 1.01 and 1.69
    ...
    # resizing kinetic energies for plotting
